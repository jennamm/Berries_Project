...
server <- function(input, output) {
    ...
    output$summary <- renderTable({
        if(isTRUE(input$group=="Chemical Type")){
            table <- ras_eda %<>% group_by(Chemical_Type) %>% summarise(mean(Value, na.rm = TRUE))
        }
        if(isTRUE(input$group=="Year")){
            table <- ras_eda %<>% group_by(Year) %>% summarise(mean(Value, na.rm = TRUE))
        }
        if(isTRUE(input$group=="State")){
            table <- ras_eda %<>% group_by(State) %>% summarise(mean(Value, na.rm = TRUE))
        }
        table
    })
}
